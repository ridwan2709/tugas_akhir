  ...
  <div class="conty">
    ...
    <div class="content-box">
      <div class="element-header" style="text-align: center; padding-top: 20px;">
        <a href="<?= base_url('parents/karakter_building/') ?>">
      ...
      <div class="navigation-arrow mb-2">
        <div class="arrow-left mr-2" style="float: left;">
          <a href="<?= base_url('parents/karakter_building/' . ($week ? $week - 1 : '-1')) ?>" class="btn btn-block btn-outline-primary withripple">Sebelumnya</a>
        </div>
        <!-- <div class="arrow-today mr-2">
          <a href="<?= base_url('parents/karakter_building/') ?>" class="btn btn-block btn-primary withripple">Minggu ini</a>
        </div> -->
        <div class="arrow-right mr-2" style="float: right;">
          <a href="<?= base_url('parents/karakter_building/' . ($week ? $week + 1 : '1')) ?>" class="btn btn-block btn-outline-primary withripple">Selanjutnya</a>
        </div>
      </div> <br /><br /><br />
      <div class="os-tabs-w">
        <div class="os-tabs-controls">
                ...
                <table class="table table-padded">
                  ...
                  <tbody>
                    ...
                    ?>
                      <tr>
                        ...
                        <td class="cell-with-media">
                          <a href="<?php echo base_url(); ?>parents/looking_karakter/<?php echo $row2['student_id']; ?><?= $week ? "/$week" : "" ?>" style="color:#999999"><img alt="" src="<?php echo $this->crud_model->get_image_url('student', $row2['student_id']); ?>" style="height: 25px; border-radius:50%" class="purple"><span> <?php echo $this->crud_model->get_name('student', $row2['student_id']); ?></span></a>
                        </td>
                    ...
                    foreach ($result as $row) {
                      ...
                      $cek_data = $this->db->get_where('build', array('user_id' => 'parent-'.$this->session->userdata('login_user_id'), 'date' => $date, 'student_id' => $row['student_id']))->row();
                    ?>
                      <tr>
                        <td><span><?php echo $this->db->get_where('enroll', array('student_id' => $row['student_id']))->row()->roll; ?></span></td>
                        <td class="cell-with-media">
                          <a href="<?php echo base_url(); ?>parents/looking_karakter/<?php echo $row['student_id']; ?><?= $week ? "/$week" : "" ?>" style="color:#999999"><img alt="" src="<?php echo $this->crud_model->get_image_url('student', $row['student_id']); ?>" style="height: 25px; border-radius:50%;" class="purple"><span> <?php echo $this->crud_model->get_name('student', $row['student_id']); ?></span></a>
                        </td>
...
<script type="text/javascript">
  ...
  function get_class_students(class_id) {
    $.ajax({
      url: '<?php echo base_url(); ?>parents/get_class_stundets/' + class_id,
      success: function(response) {
