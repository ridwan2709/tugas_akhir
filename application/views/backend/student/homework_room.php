...
foreach ($current_homework as $row) :
	$query = $this->db->get_where('deliveries', array('homework_code' => $homework_code, 'student_id' => $this->session->userdata('login_user_id')));
?>
	<link href="<?php echo base_url();?>uploads/fonts/font-fileuploader.css" rel="stylesheet">
    <link href="<?php echo base_url();?>uploads/fonts/script.css" media="all" rel="stylesheet">
	<div class="content-w">
		<div class="conty">
					...
					<div class="row">
						<div class="col-sm-8">
							<div class="pipeline white lined-primary shadow">
								...
								<?php endif; ?>
								<?php if ($row['type'] == 2 && $query->num_rows() <= 0) : ?>
									<div class="b-t padded-v-big">
								...
								<?php if ($row['type'] == 1 && $query->num_rows() <= 0) : ?>
									...
									<div class="b-t padded-v-big">
										<textarea cols="80" id="ckeditor1" name="reply" rows="10"></textarea>
										<input type="hidden" name="class_id" value="<?php echo $row['class_id']; ?>">
										<input type="hidden" name="section_id" value="<?php echo $row['section_id']; ?>">
										<input type="hidden" name="subject_id" value="<?php echo $row['subject_id']; ?>">
										<br>
								...
								<?php if ($row['type'] == 3 && $query->num_rows() <= 0) : ?>
									<div class="b-t padded-v-big">
										<input type="hidden" name="class_id" value="<?php echo $row['class_id']; ?>">
										<input type="hidden" name="section_id" value="<?php echo $row['section_id']; ?>">
										<input type="hidden" name="subject_id" value="<?php echo $row['subject_id']; ?>">
									...
									<table class="table table-lightbor table-lightfont">
										...
										<tr>
											...
											<td>
												...
												<?php endif; ?>
												<?php if ($query->num_rows() > 0) : ?>
													<a class="btn btn-rounded btn-sm btn-primary" style="color:white"><?php $mark = $this->db->get_where('deliveries', array('homework_code' => $homework_code, 'student_id' => $this->session->userdata('login_user_id')))->row()->mark;
																														if ($mark > 0) echo $mark;
																														else echo get_phrase('waiting_results'); ?></a>
												<?php endif; ?>
											</td>
										</tr>
										<tr>
	...
	<script src="<?php echo base_url();?>uploads/jquery.fileuploader.min.js"></script>
    <script type="text/javascript">
        $(document).ready(function() {
    	    $('input[id="file"]').fileuploader({
        		limit: 20,
    		    maxSize: 50,
    		    addMore: true,
                thumbnails: {
