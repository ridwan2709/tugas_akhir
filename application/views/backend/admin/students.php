																	...
																	<ul class="navs navs-tabs upper" style="padding-left:20px; padding-top:20px">
																		...
																		if ($query->num_rows() > 0) :
																			...
																			foreach ($sections as $rows) : ?>
																				<li class="navs-item">
																					<a class="navs-link" style="color:#000;" data-toggle="tab" href="#tab<?php echo $rows['section_id']; ?>"><?php echo get_phrase('section'); ?> <?php echo $rows['name']; ?></a>
																				</li>
																			<?php endforeach; ?>
																		<?php endif; ?>
																	</ul>
																...
																<div class="row" id="results">
																	<?php if ($students = $this->db->get_where('enroll', array('class_id' => $class_id, 'year' => $running_year))->num_rows() > 0) : ?>
																		...
																		foreach ($students as $row) : 
																			...
																			$status = $this->db->get_where('student', array('student_id'=>$row['student_id']))->row('idn');
																		?>
																				...
																				<div class="card-box widget-user ui-block list">
																					<div class="more" style="float:right;">
																						...
																						<ul class="more-dropdown">
																							<li><a href="javascript:void(0);" onclick="showAjaxModal('<?php echo base_url(); ?>modal/popup/modal_estudiante/<?php echo $row['student_id']; ?>');"><?php echo get_phrase('edit'); ?></a></li>
																							<li><a class="delete" href="<?php echo base_url(); ?>admin/delete_student/<?php echo $row['student_id']; ?>"><?php echo get_phrase('delete'); ?></a></li>
																							 <?= $status != 1 ? '<li><a href="'.base_url().'admin/student/insert_idn/'.$row['roll'].'">Hubungkan ke IDN</a></li>' : '' ?>
																						</ul>
																					</div>
																					<div>
																						<img src="<?php echo $this->crud_model->get_image_url('student', $row['student_id']); ?>" class="img-responsive rounded-circle purple" alt="user">
																						<div class="wid-u-info">
																							<a href="<?php echo base_url(); ?>admin/student_portal/<?php echo $row['student_id']; ?>/" class="h6 author-name">
																								<h5 class="mt-0 m-b-5"> <?php echo $this->crud_model->get_name('student', $row['student_id']); ?> <?= $status == 1 ? '<i class="mdi mdi-check-circle text-success"></i>' : '' ?></h5>
																							</a>
																							<p class="text-muted m-b-5 font-13"><b><i class="picons-thin-icon-thin-0291_phone_mobile_contact"></i></b> <?php echo $this->db->get_where('student', array('student_id' => $row['student_id']))->row()->phone; ?><br>
																								...
																								<b><i class="picons-thin-icon-thin-0729_student_degree_science_university_school_graduate"></i></b> <span class="badge badge-primary" style="font-size:10px;"><?php echo $this->db->get_where('class', array('class_id' => $row['class_id']))->row()->name; ?> - <?php echo $this->db->get_where('section', array('section_id' => $row['section_id']))->row()->name; ?></span><br>
																							</p>
																						</div>
																					</div>
																				</div>
																						...
																						<div class="card-box widget-user ui-block list">
																							<div class="more" style="float:right;">
																								...
																								<ul class="more-dropdown">
																									...
																									<li><a class="delete" href="<?php echo base_url(); ?>admin/delete_student/<?php echo $row2['student_id']; ?>"><?php echo get_phrase('delete'); ?></a></li>
																								</ul>
																							</div>
																							<div>
																								<img src="<?php echo $this->crud_model->get_image_url('student', $row2['student_id']); ?>" class="img-responsive rounded-circle purple" alt="user">
																								<div class="wid-u-info">
