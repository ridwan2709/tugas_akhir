                  ...
                  <tbody>
                    <?php foreach ($students_array as $row) : ?>
                      <tr>
                            ...
                            if ($query->num_rows() > 0) {
                              ...
                              if ($status == 'fail') {
                                if ($waktu_saat_ini >= $waktu_mulai && $waktu_saat_ini <= $waktu_akhir) {
                                  echo "<a href='" . base_url() . "admin/update_attended/" . $online_exam_id . "/" . $row['student_id'] . "' onclick=\"alert('Yakin ingin reset ?')\" ><span class='badge badge-danger'>Gagal</span></a>";
                                } else {
                                  echo "<span class='badge badge-danger'>Gagal</span>";
                                }
                              } else if ($status == 'pass') {
                                if ($waktu_saat_ini >= $waktu_mulai && $waktu_saat_ini <= $waktu_akhir) {
                                  echo "<a href='" . base_url() . "admin/update_attended/" . $online_exam_id . "/" . $row['student_id'] . "' onclick=\"alert('Yakin ingin reset ?')\" ><span class='badge badge-info'>Lulus</span></a>";
                                } else {
                        ...
                        <td><?php if ($query->num_rows() > 0) { ?><a href="<?php echo base_url(); ?>admin/online_exam_result/<?php echo $online_exam_id; ?>/<?php echo $row['student_id']; ?>/" class="btn btn-success btn-sm btn-rounded"><?php echo 'lihat jawaban siswa'; ?></a><?php } else echo ''; ?></td>
                      </tr>
                    <?php endforeach; ?>
                  </tbody>
                </table>
