                ...
                <div class="ui-block">
                  ...
                  <div class="ui-block-content">
                   <div class="table-responsive">
                    <div id="tagihan_yang_belum_dibayar">
                      <?php
                      $query_st = $this->db->query("SELECT invoice_id, student_id, SUM(amount) AS sisa_tagihan FROM `invoice` WHERE `year` = '" . $running_year . "' AND `student_id` = '" . $row['student_id'] . "' AND status = 'pending' ORDER BY `creation_timestamp` DESC");
                      $row_st = $query_st->row();
                      $sisa_tagihan = 0;

                      if (isset($row_st)) {
                        $sisa_tagihan = $row_st->sisa_tagihan;
                      }
                      if ($sisa_tagihan > 0) {
                      ...
                      ?>
                      <h6 class="title" style="text-align: center;">Sisa Tagihan : <span class="<?= $class_saldo ?>">Rp. <?= number_format($sisa_tagihan) ?></span></h6>
                    </div>
                      ...
                      <tbody>
                        ...
                        foreach($invoices as $row2): ?>
                          <tr>
                            ...
                            <td><a class="badge badge-primary" href="javascript:void(0);"><?php echo $this->db->get_where('settings' , array('type'=>'currency'))->row()->description; ?><?php echo number_format($row2['amount']);?></a></td>
                          </tr>
                        <?php endforeach;?>
                      </tbody>
                    </table>
