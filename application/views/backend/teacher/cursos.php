                              ...
                              foreach ($sections as $row) : $active2++; ?>
                                <div class="tab-pane <?php if ($active2 == 1) echo "active"; ?>" id="tab<?php echo $row['section_id']; ?>">
                                  <div class="row">
                                    ...
                                    $subjects = $this->db->get_where('subject', array('class_id' => $cl_id, 'teacher_id' => $this->session->userdata('login_user_id')))->result_array();
                                            ...
                                            <div class="friend-item-content">
                                              <div class="more">
                                                ...
                                                <ul class="more-dropdown">
                                                  <li><a href="<?php echo base_url(); ?>teacher/subject_dashboard/<?php echo base64_encode($row2['class_id'] . "-" . $row['section_id'] . "-" . $row2['subject_id']); ?>/"><?php echo get_phrase('dashboard'); ?></a></li>
                                    ...
                                    <?php endforeach; ?>
                                    <?php
                                    $q_rel = $this->db->select('rmg.*, s.icon, s.name, s.class_id, s.color')->from('relasi_mapel_guru rmg')->join('subject s', 's.subject_id = rmg.subject_id')->where(array(
                                      's.class_id' => $cl_id,
                                      'rmg.teacher_id' => $this->session->userdata('login_user_id'),
                                    ));
                                    $this->db->group_by('s.subject_id');
                                    $this->db->order_by('s.subject_id', 'desc');
                                    $subjects_rel = $q_rel->get();
                                    foreach ($subjects_rel->result_array() as $row3) :
                                    ?>
                                      <div class="col col-xl-3 col-lg-6 col-md-6 col-sm-12 col-12">
                                        <div class="ui-block" data-mh="friend-groups-item">
                                          <div class="friend-item friend-groups">
                                            <div class="friend-item-content">
                                              <div class="more">
                                                <i class="icon-feather-more-horizontal"></i>
                                                <ul class="more-dropdown">
                                                  <li><a href="<?php echo base_url(); ?>teacher/subject_dashboard/<?php echo base64_encode($row3['class_id'] . "-" . $row['section_id'] . "-" . $row3['subject_id']); ?>/"><?php echo get_phrase('dashboard'); ?></a></li>
                                                </ul>
                                              </div>
                                              <div class="friend-avatar">
                                                <div class="author-thumb">
                                                  <img src="<?php echo base_url(); ?>uploads/subject_icon/<?php echo $row3['icon']; ?>" width="120px" style="background-color:#<?php echo $row3['color']; ?>;padding:30px;border-radius:0px;">
                                                </div>
                                                <div class="author-content">
                                                  <a href="<?php echo base_url(); ?>teacher/subject_dashboard/<?php echo base64_encode($row3['class_id'] . "-" . $row['section_id'] . "-" . $row3['subject_id']); ?>/" class="h5 author-name"><?php echo $row3['name']; ?> - <?php echo $row['name']; ?></a><br><br>
                                                  <img src="<?php echo $this->crud_model->get_image_url('teacher', $row3['teacher_id']); ?>" style="border-radius:50%;width:20px;"><span> <?php echo $this->crud_model->get_name('teacher', $row3['teacher_id']); ?></span>
                                                </div>
                                              </div>
                                            </div>
                                          </div>
