...
class go2hi
{
	...
	protected static $monthA = array('\M\u\h\a\r\r\a\m', '\S\af\a\r', 'R\ab\i\'\u\l \A\w\a\l', 'R\a\b\i\'\u \T\s\a\a\n\i', 'J\u\m\a\d\i\l \A\w\a\l', 'J\u\m\a\d\i\l \Ak\h\i\r', 'R\a\j\ab', '\S\y\a\'b\a\n', 'R\a\m\a\d\h\a\n', '\S\y\a\w\a\l', '\Dz\u\l Q\a\'\d\a\h', '\Dz\u\l \H\i\j\j\a\h');
	...
	protected static $dayA = array('\K\h\a\m\i\s', '\J\u\m\u\'\a\h', '\S\a\b\t', '\A\h\a\d', '\I\t\s\n\a\y\n', 'T\s\u\l\a\t\s\a\'', '\A\r\b\i\'\a\'');
	...
	private static $hijri_date_correction = 0;
	...
	public static function date($format, $ctype = 0, $timestamp = 0, $lang = 0)
	{
		...
		switch ($ctype) {
			...
			case 1:
				...
				break;
			default:
				trigger_error('Calender type doesnt match in go2hi::date()', E_USER_ERROR);
		}
		$lang = intval($lang);
		switch ($lang) {
			case 0:
			case 1:
				...
				break;
			default:
				trigger_error('Language doesnt exists in go2hi::date()', E_USER_ERROR);
		}
		self::reset_vars();

		$time = intval($timestamp);
		if (!empty($timestamp)) : self::$time = $time;
		else : self::$time = time();
		endif;
