...
class Student extends CI_Controller
{
    ...
    function meet($task = "", $document_id = "")
    {
        ...
        $data['page_name']              = 'meet';
        $data['data']              = $task;
        $data['page_title']             = get_phrase('live');
    ...
    function liveClass($live_id = '', $liveType = '')
    {
        if ($this->session->userdata('student_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(base_url(), 'refresh');
        ...
        if (!$query) {
            $this->academic_model->saveAttendance($class_id, $section_id, $subject_id, $year, $timestamp, $live_id);
        } else {
            $this->academic_model->updateAttendance($class_id, $section_id, $subject_id, $year, $timestamp, $live_id);
        }
    ...
    function take_online_exam($online_exam_code  = '')
    {
        ...
        $online_exam_id = $this->db->get_where('online_exam', array('code' => $rand, 'password' => $pass))->row()->online_exam_id;
    ...
    function my_profile($param1 = '', $param2 = '')
    {
        ...
        if ($param1 == 'update') {
            ...
            $data['authorized_person'] = $this->input->post('authorized_person');
            $data['authorized_phone'] = $this->input->post('authorized_phone');
    ...
    function attendance_report_selector()
    {
        ...
        redirect(base_url() . 'student/report_attendance_view/' . $data['data'] . '/' . $data['month'] . '/' . $data['year'], 'refresh');
    }
    ...
    function subject($param1 = '', $param2 = '')
    {
        ...
        $student_class_id        = $this->db->get_where('enroll', array(
            'student_id' => $student_profile->student_id, 'year' => $this->db->get_where('settings', array('type' => 'running_year'))->row()->description
        ))->row()->class_id;
    ...
    function invoice($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'make_payment') {
            ...
            $student_profile         = $this->db->get_where('enroll', array('student_id'   => $this->session->userdata('student_id')))->row();
            ...
            redirect('https://infradigital.io/bills?biller_code=10038&bill_key=' . $student_profile->roll, 'refresh');
        }
    ...
    function delivery($param1 = '', $param2 = '')
    {
        ...
        if ($param1 == 'file') {
            $this->academic_model->sendFileHomework($param2);
            $this->session->set_flashdata('flash_message', get_phrase('successfully_updated'));
    ...
    function blog($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $data['tulisan_author']       = $this->crud_model->get_name('student', $this->session->userdata('login_user_id'));
            ...
            $this->session->set_flashdata('flash_message', get_phrase('successfully_added'));
            redirect(base_url() . 'student/blog/', 'refresh');
        }
        if ($param1 == 'edit') {
            $data['tulisan_judul']       = $this->input->post('judul');
            $data['tulisan_isi']       = $this->input->post('isi');
